...
int AC_Trie:: InsertWord(int* Word, int Number)
{
	...
	{ 
		...
		Childs[k]->InsertWord(Word+1, Number);
	}
	return 0;
}
...
void preLink()
{
	...
	for ( i = 0; i < MainData::AlpSize; i++)
		if (AC_Trie::gTrie->Childs[i] != NULL)
			queue.push_back(AC_Trie::gTrie->Childs[i]);	

	while (queue.size() > 0) //queue is not empty
	{
		AC_Trie *v1= queue[0]; //pop				
		queue.pop_front();
		int x;
...
void AC_Trie:: PrintWay(ofstream *ff){
	if(lp!=0){
		Parent->PrintWay(ff);
...
//It Prints a subtrie
void AC_Trie:: PrintTrie(ofstream *ff){
	int i;
	if(iEnd!=0){
		PrintWay(ff);
		*(ff)<<'\n';
