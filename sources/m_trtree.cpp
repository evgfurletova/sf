...
void M_TrTree::FarPartProbs(double** MasPrProbs){
	...
	if((iEnd == 0)&&(len > 0)){
		MaseProbs = new double*[NStates]; 
		for(i = 0; i < NStates; i++){
			MaseProbs[i] = new double[MainData::NOccur];
			for(j = 0; j < MainData::NOccur; j++){
				MaseProbs[i][j] = 0;
			}
		}
	...
	if(iEnd != 0){
		if(len > MainData::order){
			for(i = 0; i < MainData::NOccur; i++){
				for(j = 0; j < NStates; j++){
					H_M_OVGraf::FarProbs[iEnd - 1][i][Positions[j]] += MasPrProbs[0][i]*Probs[j];
				}
			}
		}
		else{
			...
			for(i = 0; i < MainData::NOccur; i++){
				...
				for(l = 0; l < s; l++){
				    cd = Parent->States[l];
					H_M_OVGraf::FarProbs[iEnd - 1][i][0] += MasPrProbs[l][i]*MainData::MarkovProbs[pos][cd];
				}
			}
		}
	}
