...
{
    self = self;
    static char *kwlist[] = {"AlpMas", "order", "BernProb", NULL };
    int AlpSize = 0;
    char *AlpMas = NULL;
    int order = 0;
    double *BernProb = NULL;
    PyObject * l = NULL;
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "siO", kwlist, &AlpMas, &order, &l))
        return NULL;
    ...
    int ret = func_analis_alp_bern_data(AlpSize, AlpMas, order, BernProb);
    PyObject* result = Py_BuildValue("i", ret);
    return result;
}
...
{
    self = self;
    static char *kwlist[] = {"AlpMas", "MarkovProbs", NULL };
    int AlpSize = 0;
    char *AlpMas = NULL;
    ...
    PyObject * ll = 0;
    PyObject * current = 0;
    MarkovProbs = (double**)calloc(AlpSize, sizeof(double*));
    for (i=0; i<AlpSize; i++) {
    ...
    int ret = func_analis_alp_mark_data(AlpSize, AlpMas, MarkovProbs);
    for (i=0; i<AlpSize; i++) {
        free(MarkovProbs[i]);
    }
    free(MarkovProbs);
...
extern PyObject* py_func_main(PyObject *self, PyObject *args, PyObject *keywds)
{
    ...
    keywds = keywds;
    double pvalue = 0.0;
    int ret = func_main(&pvalue);
    PyObject* result = Py_BuildValue("id", ret, pvalue);
    return result;
}
